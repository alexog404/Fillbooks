 ...
 * so any cached row inside the range is treated as proof the whole range
 * was already fetched -- no per-bar freshness check. A fetch that comes
 * back empty (old trade beyond the provider's history limit) is
 * deliberately *not* cached as a "known miss": at this data volume/audience
 * of one, refetching on the next visit is cheap and simpler than adding a
 * fetch-attempted marker to the schema.
 */
export async function getBarsForSession(
  ...
  if (cached.length > 0) return rowsToBars(cached);
