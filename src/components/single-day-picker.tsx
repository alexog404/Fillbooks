...
export function SingleDayPicker({
  ...
  return (
    <div className="absolute left-0 top-[34px] z-10 bg-surface border border-border rounded-[10px] p-3 shadow-[0_8px_24px_rgba(0,0,0,0.35)] w-[230px] select-none">
      <div className="text-[9.5px] text-text-muted mb-1.5">Select a day</div>
      <div className="flex items-center justify-between mb-2">
        <button type="button" onClick={() => onMonthChange(shiftMonthKey(monthKey, -1))} className="w-[22px] h-[22px] rounded-md bg-surface-2 flex items-center justify-center cursor-pointer text-[11px]">‹</button>
