...
function BarRow({ label, pct, pctText, color }: { label: string; pct: number; pctText: string; color: string }) {
  return (
    <div className="flex items-center gap-2.5 mb-2.5">
      <div className="w-[110px] text-[11.5px] text-text-secondary">{label}</div>
      <div className="flex-1 h-2.5 rounded-full bg-surface-2 overflow-hidden">
        <div className="h-full" style={{ width: `${pct}%`, background: color }} />
      </div>
      <div className="w-11 text-right text-[11px] font-mono text-text-secondary">{pctText}</div>
    </div>
  );
}
    ...
    return (
      <div className="px-7 pt-6 pb-10">
        ...
        <div className="border border-dashed rounded-xl px-6 py-14 text-center text-text-muted" style={{ borderColor: "var(--border-strong)" }}>
          ...
          <div className="text-sm font-semibold text-text-secondary">Not enough trades yet</div>
          <div className="text-xs mt-1">Insights need a larger sample size to be meaningful — log more trades to unlock this view.</div>
        </div>
      </div>
    );
  }
    ...
    <div className="px-7 pt-6 pb-10">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="relative">
          <button type="button" onClick={() => setShowPicker((v) => !v)} className="text-xs px-3 py-1.5 rounded-md bg-surface-2 border border-border text-text-secondary cursor-pointer flex items-center gap-1.5">
            📅 {rangeLabel} ▾
          </button>
          {showPicker && (
            <DateRangePicker
      ...
      {reportsTrades.length === 0 ? (
        <div className="border border-dashed rounded-xl px-6 py-14 text-center text-text-muted" style={{ borderColor: "var(--border-strong)" }}>
          <div className="text-2xl mb-2.5">▤</div>
          <div className="text-sm font-semibold text-text-secondary">No trades in this range</div>
        </div>
      ) : (
        <>
              ...
              <div key={i} className="grid grid-cols-3 border-t border-border">
                {row.map((cell, j) => (
                  <div key={j} className="flex items-center justify-between gap-2.5 px-4.5 py-2.5 border-l border-border first:border-l-0">
                    <div className="text-[12.5px] text-text-muted">{cell.label}</div>
                    <div className="text-[13.5px] font-bold font-mono whitespace-nowrap" style={cell.color ? { color: cell.color } : undefined}>{cell.value}</div>
                  </div>
                ))}
              </div>
            ))}
            ...
            {rStats === null || rHistogram === null ? (
              ...
              <>
                <div className="grid gap-3 mb-4.5" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(110px, 1fr))" }}>
                  {rStats.map((rs) => (
                    <div key={rs.label}>
                      <div className="text-[10.5px] text-text-muted uppercase tracking-[0.04em]">{rs.label}</div>
                      <div className="text-[17px] font-bold font-mono mt-1" style={{ color: rs.color }}>{rs.value}</div>
                    </div>
                  ))}
                </div>
                <div className="text-[11px] text-text-muted mb-2">Average R by trading day — spot your worst days</div>
                <div className="flex items-end gap-1.5 h-[130px] overflow-x-auto pb-0.5">
                  {rHistogram.map((rh, i) => (
                    <div key={i} className="flex-none w-[34px] flex flex-col items-center justify-end h-full gap-1">
                      <div className="text-[9.5px] text-text-secondary font-mono whitespace-nowrap">{rh.avgR.toFixed(2)}</div>
                      <div className="w-full rounded" style={{ height: `${rh.barHeightPct}%`, minHeight: 2, background: rh.color }} />
                      <div className="text-[8.5px] text-text-muted text-center whitespace-nowrap" style={{ transform: "rotate(-40deg)", transformOrigin: "top center", marginTop: 4 }}>
                        {rh.label}
                      </div>
                    </div>
                  ))}
          ...
          <div className="bg-surface border border-border rounded-[10px] p-4">
            ...
            {byPrice.map((b) => (
              <div key={b.label} className="flex items-center gap-2.5 mb-2.5">
                ...
                <div className="w-[60px] text-right text-[11px] font-mono text-text-secondary">{b.shares.toLocaleString()} sh</div>
              </div>
            ))}
          </div>
        </>
