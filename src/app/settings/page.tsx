...
export default async function SettingsPage({
  ...
  return (
    <div className="px-7 pt-6 pb-10 max-w-[1040px]">
      ...
      <div className="text-[15.5px] text-text-muted mb-[18px]">Connections, accounts, and how the app looks</div>

      {params.schwab_connected && (
        <div className="text-[15.5px] mb-4 px-4 py-2.5 rounded-lg" style={{ color: "var(--win)", background: "var(--win-soft)" }}>
          Schwab connected successfully.
        </div>
      )}
      {params.schwab_error && (
        <div className="text-[15.5px] mb-4 px-4 py-2.5 rounded-lg" style={{ color: "var(--loss)", background: "var(--loss-soft)" }}>
      ...
      <div className="bg-surface border border-border rounded-[10px] px-5 py-[18px]">
        ...
        <div className="flex items-center justify-between py-2.5 border-t border-border">
          <div>
            <div className="text-[16px] font-semibold">Display timezone</div>
            <div className="text-[14.5px] text-text-muted mt-0.5">Used for the secondary &quot;local&quot; time everywhere</div>
          </div>
          <TimezonePicker />
        </div>
        <div className="flex items-center justify-between py-3 border-t border-border">
          <div>
            <div className="text-[16px] font-semibold">Colorblind-safe palette</div>
            <div className="text-[14.5px] text-text-muted mt-0.5 max-w-[220px]">P&amp;L is this app&apos;s most important signal — swap it to blue/orange</div>
          </div>
          <ColorblindToggle />
        </div>
      </div>
